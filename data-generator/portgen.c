...
{
	int graph_number;
	graph_number = 10000;  // number of graph instances to generate
	int id;
	for (id = 1; id <= graph_number; id++) {
		...
		N = 5;  // number of vertices in the graph instance
		seed = id + 10000123;  // random seed
		...
		FILE *fp = NULL;
		strcpy(filename, "tsp5_r_test_1w.txt");  // storage path of the graph instances
		fp = fopen(filename, "a+");
		//printf("%s", filename);
