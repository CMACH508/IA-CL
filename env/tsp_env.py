...
class TSPEnv(gym.Env, StaticEnv):
    ...
    def __init__(self, graph: Graph, config):
        self.graph = graph
        self.config = config
    ...
    def get_obs_for_states(states):
        ...
        for state in states:
            ...
            node_attr = np.zeros((ver_num, state['config']['arch']['args']['node_dim']), dtype=np.float)
            node_attr[:, 0] = state['visited']
            node_attr[:, 1:3] = state['graph'].ver_coo
            node_attr[:, 3] = 1
            node_attr[:, 4:6] = state['graph'].ver_coo[state['tour'][0]]

            edge_attr = np.zeros((ver_num, state['config']['data_loader']['data']['knn'], state['config']['arch']['args']['edge_dim']), dtype=np.float)
            ...
            edge_attr = torch.tensor(edge_attr, dtype=torch.float).view(-1, state['config']['arch']['args']['edge_dim'])
